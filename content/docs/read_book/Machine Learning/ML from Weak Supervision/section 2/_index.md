...
今までの議論は二値分類だったので、これを多クラス分類に拡張する。$\mathbf{x}$のデータを$y \in 1, 2, \cdots, c$に分類させる。
分類器は$g(\mathbf{x})$は、二値分類の時はスカラーを出力し、正負でラベルが$y = +1, -1$であるか、を比較すればよかった。これを多次元に拡張すると、以下のようにベクトル状にして、**各要素にはそのカテゴリに該当する度合(確率ではない！)**が与えられる。

$$
g(\mathbf{x}) = (g _1(\mathbf{x}), \cdots, g _c (\mathbf{x})) ^ T
$$

理想的な損失関数は二値分類の時と同様に01損失で定義できる。

## 代理損失関数 in 多クラス分類

当然、これも定義することになる。多次元分類における主な損失関数は以下の通り。二値分類に比べて少なめと言える。

![](multiclass_surrogate_loss.png)

## 訓練データ in 多クラス分類

二値分類でのpositive, negativeが従う分布は定義できるように、多次元版でも定義できる。
そしてこれらを使うことで、unlabeledの分布を合成もできる。

$$
[ \mathbf{x} _i ] _{i = 1} ^ {N _1} \overset{i.i.d}{\sim} p _1(\mathbf{x}) = p(\mathbf{x} | y = +1) \\\\ 
\cdots \\\\ 
[ \mathbf{x} _i ] _{i = 1} ^ {N _c} \overset{i.i.d}{\sim} p _c(\mathbf{x}) = p(\mathbf{x} | y = +c) \\\\ 
[ \mathbf{x} _i ] _{i = 1} ^ {N _U} \overset{i.i.d}{\sim} p _U(\mathbf{x}) = \sum _{i = 1} ^ c p(y = +i) p _i(\mathbf{x})
$$

これも二値分類と同様に、不偏推定量で推定することに変わりはない。正則化項については言及されてないが、二値分類と同じだろう。
